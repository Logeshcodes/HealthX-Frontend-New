...
const ReportManagement: React.FC = () => {
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await getAllReports(currentPage , reportPerPage, searchTerm);
            ...
            <table className="min-w-full bg-gray-800 rounded-lg overflow-hidden">
              <thead className="bg-gray-700">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Doctor Details</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Patient Details</th>
              ...
              const doctor = doctors.find((doc : any) => doc._id === report.doctorId); 
              const user = users.find((doc : any) => doc._id === report.userId); 

  return (
    <tr key={report._id} className="hover:bg-gray-700 transition">
      ...
      <td className="px-6 py-4">
        ...
        <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center overflow-hidden m-4">
          ...
          <div>
            <p className="font-medium">{doctor?.name } ({doctor?.department || "N/A"})</p>
            <p className="text-xs text-gray-500">{doctor?.email || "N/A"}</p>
          </div>
        </div>
      </td>
      <td className="px-6 py-4">
      ...
      <td className="px-6 py-4">
        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          report.reportType === 'Inappropriate Behavior' ? 'bg-red-900 text-red-200' :
          report.reportType === 'Misdiagnosis' ? 'bg-yellow-900 text-yellow-200' : 
