...
const AdminDashboard: React.FC = () => {
  ...
  // Generate mock revenue data based on appointments and timeFilter
  const generateRevenueData = (appointments: Appointment[], filter: 'day' | 'month' | 'year') => {
    if (!appointments || appointments.length === 0) return [];

    // Group appointments by date and sum the amounts
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      setLoading(true);
      try {
        // Using the existing API functions as shown in your code snippets
        ...
        // Generate revenue data based on appointments
  ...
  // Calculate dashboard counts from fetched data
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}
                  >
                    {prepareDoctorStatusData().map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                    ))}
                  </Pie>
                  <Tooltip 
...
const getRevenueData = async (timeFilter: 'day' | 'month' | 'year') => {
  try {
    const response = await axios.get(`/api/admin/revenue?timeFilter=${timeFilter}`);
